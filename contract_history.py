...
def scan_block(fromBlock=1, max=0):
    ...
    for i in range(fromBlock, maxBlockNum):
        ...
        if len(block['transactions']) > 0:
            tx_count = 0
            tx_prefix = block['transaction_ids'][tx_count]+','+str(i)
            for t in block['transactions']:
                for op in t['operations']:
                    if op[0] == 76: # contract_register_operation
                        # f.write(tx_prefix+',deposit,'+op[1]['cross_chain_trx']['asset_symbol']+','+str(op[1]['cross_chain_trx']['amount'])+'\n')
                        logging.debug(tx_prefix+','+str(op))
                    elif op[0] == 77: # contract_upgrade_operation
                        # f.write(tx_prefix+',withdraw,'+op[1]['asset_symbol']+','+str(op[1]['amount'])+'\n')
                        logging.debug(tx_prefix+','+str(op))
                    elif op[0] == 78: # native_contract_register_operation
                        # f.write(tx_prefix+','+op[1]['account']+','+str(op[1]['new_options']['miner_pledge_pay_back'])+'\n')
                        logging.debug(tx_prefix+','+str(op))
